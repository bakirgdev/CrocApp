...
- Dock drops during a transfer surface only after the transfer ends (staged URLs wait in `pendingSendURLs`); no feedback at drop time — a banner or toast is deferred polish (`../known-issues.md`).
...
- `ExportOptions-MAS.plist` is committed but unvalidated until the first real MAS export.
- AutoVerify + `forceLocalOnly` compile into Release; both must be gated behind a build flag before store submission (`../known-issues.md`).
