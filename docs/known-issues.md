...
- **`poll()` marshals progress JSON on every 100 ms tick**, even when nothing changed.
- **`Crocmobile.objc.h` declares `- (nullable instancetype)init` on `CrocmobileOptions`**, which conflicts with `NSObject`'s nonnull `init` and emits two build warnings. Generated by gomobile on every `build-xcframework.sh` run, not repo code — already flagged as unfixable in `knowledge/tooling.md`.
