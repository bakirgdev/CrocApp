...
Facts verified against croc **v10.5.0**, the pinned version — source read, CLI quirks reproduced. Upstream released v10.6.0 on 2026-07-26 and the pin has not moved yet; run `croc-upgrade-playbook.md` before trusting any of this against a newer croc.

Source: https://github.com/schollz/croc

## Identity

CLI tool to "easily and securely send things from one computer to another": any file/folder/text, between any two machines, via a short code phrase. Author Zack Scholl (schollz). Go, MIT license, very actively maintained, one of the larger Go CLI projects on GitHub. Single static binary, zero dependencies, no port forwarding needed. Inspired by magic-wormhole; adds resumability, multi-file transfers, and full-duplex relay comms.
...
4. **Encryption**: end-to-end, and the live path is narrower than `src/crypt` suggests. The PAKE session key is stretched by **PBKDF2-HMAC-SHA256, 100 iterations**, to 32 bytes (`crypt.New`), and everything on the wire is **AES-256-GCM** (`crypt.Encrypt`/`Decrypt`, random 12-byte IV prepended). `crypt.NewArgon2` (Argon2id → XChaCha20-Poly1305) and `EncryptChaCha`/`DecryptChaCha` exist in the same file but have **no non-test call site in v10.5.0** — they are dead code, so do not describe croc as Argon2- or ChaCha-based. 100 PBKDF2 iterations is weak by itself; the security argument is that the input is already a high-entropy PAKE session key, not a password. Relay sees ciphertext only — untrusted by design.
...
- Importable as Go library: `github.com/schollz/croc/v10/src/croc` (`croc.New(croc.Options{...})`). Pure Go + x/crypto → gomobile binding plausible (untested).
- The "native Kotlin Android client" linked from croc's README (added 2026-07-20) is community-built [Dking08/croc-app](https://github.com/Dking08/croc-app), not in-repo; it wraps the CLI as a subprocess with stdout regex parsing — architecture unusable on iOS (see `docs/knowledge/prior-art.md`). No free/OSS iOS client exists — CrocApp fills a real gap.
