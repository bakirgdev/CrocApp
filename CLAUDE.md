...
- `.github/`: `workflows/ci.yml` (format, Go lint/vuln/build/vet, macOS + iOS builds), `workflows/release.yml` (checks, build, DMG, publish; dispatch rehearses, tag `v*` publishes, ADR 0032), `workflows/govulncheck.yml` (weekly scan, ADR 0018), `workflows/landing.yml` and `workflows/docs.yml` (Pages deploy; both publish the whole site via `scripts/assemble-site.sh`, ADR 0025), `workflows/site-preview.yml` (same build on `pull_request`, no deploy — catches `onBrokenLinks: throw` before it reaches a live deploy); issue and PR templates route per `ISSUE_TEMPLATE/config.yml`.
...
- `app/`: Xcode project (SwiftUI, iOS + macOS), `CrocApp.xcodeproj`, `CrocApp/` sources (`Support/` holds cross-view helpers: `DesignTokens.swift` routes the app onto `design/`, `SecurityScopedBookmark.swift` and `EngineConstraints.swift` are shared engine-adjacent helpers, `KeychainStore.swift` holds the relay password, ADR 0033), `CrocShare/` share extension, `Config/` (plists, entitlements, export options).
- `assets/`: brand art, plus `assets/screenshots/` — eighteen `<name>-light.png`/`<name>-dark.png` pairs, all regenerated by `scripts/capture-screenshots.sh`. Never hand-edit or hand-crop one; re-run the script.
