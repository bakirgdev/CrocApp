...
| Cloudflare sits in front of Pages (§5). A wrong SSL mode is an apex redirect loop; edge caching can mask a deploy. | Resolved 2026-07-25: the records are now unproxied (DNS only), so Cloudflare is out of the data path entirely and the SSL mode is irrelevant. Re-enabling the orange cloud reintroduces both failure modes. |
| **Never cancel a running Pages deploy.** Cancelling the workflow run does not cancel the *deployment*: it stays "in progress" server-side and blocks every later deploy with `Please cancel <sha> first`. Hit 2026-07-25; could not be cleared by `POST /pages/deployments/{sha}/cancel` (400 "already finished"), by numeric deployment id (404), by marking the deployment inactive, or by deleting the deployment record. | `pages.yml` now has an `if: cancelled()` step that calls the cancel API on the way out. If a lock is already stuck, the only known fix is Settings → Pages → switch Source to "Deploy from a branch", save, switch back to "GitHub Actions", save. |
